...
class VideoSynthBase(object):
    ...
    def __init__(self, size=None, noise=0.0, bg=None, **params):
        ...
        self.bg = None
        self.frame_size = (640, 480)       # 打开的框的大小

        # 读取图像。第二个参数是 1，0，-1：彩色图片，忽视它的透明度。灰度图。加载图像。BGR 格式数据格式在 0~255，通道格式为(W,H,C)
        # 即使图像路径是错误的，它也不会抛出任何错误，但是返回的会是None。
        ...
        print('size:', size)
        if size is not None:
            w, h = map(int, size.split('x'))                     # 以x为分隔符
            self.frame_size = (w, h)
            self.bg = cv2.resize(self.bg, self.frame_size)       # scr:原图 dsize：输出图像尺寸
